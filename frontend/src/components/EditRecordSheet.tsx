...
import { careRecordForm, editedRecordPayload, localFormValue } from "../careRecordForms";
...
export function EditRecordSheet({ record, onClose }: { record: TimelineRecord; onClose: () => void }) {
  ...
  const form = careRecordForm(record.record_type);
  ...
  const staleError = update.error instanceof ApiError && update.error.status === 409 && update.error.detail === "stale_revision" ? update.error : null;

  return <BottomSheet label={`Edit ${careAction(record.record_type).label}`} onClose={onClose} returnFocusSelector={`[data-record-options-id="${record.id}"]`}>
      ...
      <form onSubmit={submit} className="form-stack care-form">
        ...
        {!online ? <p className="offline-notice" role="status">Reconnect to edit this record.</p> : staleError ? <p className="error" role="alert">This record changed on another device to revision {staleError.current?.revision ?? "a newer version"} by {staleError.current?.last_modified_by_label ?? "another caregiver"}. Close this editor, review the latest version, and try again.</p> : update.isError && <p className="error" role="alert">Could not save these changes. Check the values and try again.</p>}
